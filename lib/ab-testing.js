...
module.exports = {
  createTest: function (name, config) {
    ...
    var testForUserId = function (userId, functions, that) {
      var testGroupName = getGroup(userId);
      test(testGroupName, functions, that);
    };
    ...
    .map(function (test) {
      if (test.name && typeof test.name === "string") {
        return {
          // Weight from 0..1
          weight:
            (isFinite(test.weight) ? test.weight : defaultWeight) / totalWeight,
          name: test.name,
        };
      }
    })
